...
zkps = [ZKP.Schnorr(security, confidence), ZKP.GuillouQuisquater(security, confidence), ZKP.FiatShamir(security, confidence), ZKP.ChaumPedersen(security, confidence)]
nizks = [NIZK.Schnorr(security), NIZK.GuillouQuisquater(security), NIZK.ChaumPedersen(security)]

for algorithm in (zkps + nizks):
