...
class ZeroKnowledgeProof(object):
	...
	def challenge(self, com, req):
		""" The verifier generates a challenge chg on input a commitment com and a request req. """
	...
	def demo(self, message):
		...
		for _ in range(self.confidence):
			...
			queries.append({'request': req, 'challenge': chg, 'response': res, 'verified': valid})
...
class ChaumPedersen(Groups.PrimeOrder, ZeroKnowledgeProof):
	...
	def verify(self, com, req, chg, res):
		...
		return (y1 == (req[0] * pow(com[0], chg, self.p) % self.p) and y2 == (req[1] * pow(com[1], chg, self.p) % self.p))
